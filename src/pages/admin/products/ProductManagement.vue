    ...
    <div class="space-y-6 relative min-h-screen">
        <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-sm text-gray-400 mt-1">Danh sách toàn bộ kho hàng của bạn</p>
            </div>
            <div class="flex items-center gap-3">
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" v-model="showDeleted" class="w-4 h-4 rounded accent-indigo-600" />
                    <span class="text-sm text-gray-500">Hiện đã xóa</span>
                </label>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        <th class="px-5 py-4 text-xs font-semibold text-gray-400">Sản phẩm</th>
                        <th class="px-5 py-4 text-xs font-semibold text-gray-400 hidden md:table-cell">Danh mục</th>
                        <th class="px-5 py-4 text-xs font-semibold text-gray-400 hidden lg:table-cell">Giá cơ bản</th>
                        <th class="px-5 py-4 text-xs font-semibold text-gray-400 text-center">Trạng thái</th>
                        <th class="px-5 py-4 text-xs font-semibold text-gray-400 text-right">Thao tác</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr v-for="p in filteredProducts" :key="p.product_id" :class="['hover:bg-gray-50/60 transition-colors group', p.deleted_at ? 'opacity-50' : '']">
                        <td class="px-5 py-4 cursor-pointer" @click="openDetail(p)">
                            <div class="flex items-center gap-3">
                                <div class="w-11 h-14 rounded-xl bg-gray-100 overflow-hidden shrink-0">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors line-clamp-1">{{ p.name }}</p>
                                    <p class="text-xs text-gray-400 mt-0.5">{{ p.brand || '—' }} · #{{ p.product_id }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-4 text-sm font-semibold text-gray-800 hidden lg:table-cell">
                        ...
                        <td class="px-5 py-4 text-center">
                            ...
                            <span v-else-if="!p.is_active" class="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full bg-gray-100 text-gray-500">
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="flex justify-end gap-1">
                                <button @click="openDetail(p)" class="p-2 hover:bg-indigo-50 rounded-xl transition-colors text-gray-400 hover:text-indigo-600" title="Sửa/Chi tiết">
                                    <span class="material-symbols-outlined text-[18px]">edit</span>
                                </button>
                                <button v-if="!p.deleted_at" @click="handleSoftDelete(p.product_id)" class="p-2 hover:bg-red-50 rounded-xl transition-colors text-gray-400 hover:text-red-500" title="Xóa">
        ...
        <div v-if="isDrawerOpen" class="fixed inset-0 z-[60] flex justify-end">
            ...
            <div class="relative w-full max-w-2xl bg-white shadow-2xl flex flex-col h-full rounded-l-2xl overflow-hidden">
                ...
                <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 shrink-0">
                    ...
                    <button @click="closeDrawer" class="w-8 h-8 flex items-center justify-center hover:bg-gray-200 rounded-full transition-colors">
                ...
                <div class="flex-grow overflow-y-auto p-6 space-y-8">
                    <!-- SECTION 1: ẢNH -->
                    <section>
                        <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">Hình ảnh sản phẩm</h3>
                        <input type="file" ref="fileInput" class="hidden" accept="image/*" @change="handleFileUpload" />
                        <div class="grid grid-cols-5 gap-3">
                            <div v-for="(img, idx) in selectedProduct.images" :key="idx" class="aspect-[3/4] bg-gray-100 rounded-xl relative group overflow-hidden">
                                ...
                                <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex flex-col items-center justify-center gap-2">
                                    <button v-if="!img.is_primary" @click="handleSetPrimary(img.image_id)" class="bg-white text-gray-900 text-[9px] font-semibold px-3 py-1.5 rounded-lg hover:bg-indigo-600 hover:text-white transition-colors">Đặt chính</button>
                                    <button @click="handleDeleteImage(img.image_id)" class="text-white hover:text-red-300 transition-colors">
                                        <span class="material-symbols-outlined text-sm">delete</span>
                                    </button>
                                </div>
                            </div>
                            <button @click="triggerFileInput" :disabled="isUploading" class="aspect-[3/4] border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center text-gray-400 hover:border-indigo-400 hover:text-indigo-500 transition-all disabled:opacity-50">
                    ...
                    <section>
                        <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">Thông tin cơ bản</h3>
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                <label class="text-xs font-semibold text-gray-500 block mb-1.5">Tên sản phẩm</label>
                                <input v-model="selectedProduct.name" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all" placeholder="Nhập tên sản phẩm..." />
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-500 block mb-1.5">Giá cơ bản (₫)</label>
                                <input v-model.number="selectedProduct.base_price" type="number" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all" />
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-500 block mb-1.5">Thương hiệu</label>
                                <input v-model="selectedProduct.brand" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all" placeholder="VD: Nike, Adidas..." />
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-500 block mb-1.5">Danh mục</label>
                                <select v-model="selectedProduct.category_id" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all bg-white">
                                    <option v-for="cat in allCategories" :key="cat.category_id" :value="cat.category_id">{{ cat.name }}</option>
                                </select>
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-500 block mb-1.5">Giới tính</label>
                                <select v-model="selectedProduct.gender" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all bg-white capitalize">
                                    <option v-for="g in genderOptions" :key="g" :value="g">{{ g }}</option>
                                </select>
                            </div>
                            <div class="col-span-2">
                                <label class="text-xs font-semibold text-gray-500 block mb-1.5">Mô tả</label>
                                <textarea v-model="selectedProduct.description" rows="3" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all resize-none" placeholder="Mô tả sản phẩm..."></textarea>
                            </div>
                            <div class="col-span-2">
                                <label class="text-xs font-semibold text-gray-500 block mb-2">Trạng thái</label>
                                <button @click="selectedProduct.is_active = !selectedProduct.is_active"
                                    :class="['flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all w-full text-left', selectedProduct.is_active ? 'border-emerald-200 bg-emerald-50' : 'border-gray-200 bg-gray-50']">
                                    <div :class="['w-10 h-5 rounded-full transition-colors relative shrink-0', selectedProduct.is_active ? 'bg-emerald-500' : 'bg-gray-300']">
                                    ...
                                    <span :class="['text-sm font-semibold', selectedProduct.is_active ? 'text-emerald-700' : 'text-gray-500']">
                    ...
                    <section v-if="selectedProduct.product_id">
                        <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">Biến thể sản phẩm</h3>
                        <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-4 mb-4">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                <div>
                                    <label class="text-[10px] font-semibold text-gray-500 block mb-1">Màu sắc</label>
                                    <select v-model="newVariant.color_id" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-400 bg-white">
                                        <option :value="null">Chọn màu</option>
                                        <option v-for="c in allColors" :key="c.color_id" :value="c.color_id">{{ c.name }}</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="text-[10px] font-semibold text-gray-500 block mb-1">Kích cỡ</label>
                                    <select v-model="newVariant.size_id" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-400 bg-white">
                                        <option :value="null">Chọn size</option>
                                        <option v-for="s in allSizes" :key="s.size_id" :value="s.size_id">{{ s.name }} ({{ s.size_type }})</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="text-[10px] font-semibold text-gray-500 block mb-1">SKU</label>
                                    <input v-model="newVariant.sku" placeholder="VD: SMT-RED-M" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-400" />
                                </div>
                                <div>
                                    <label class="text-[10px] font-semibold text-gray-500 block mb-1">Giá (₫)</label>
                                    <input v-model.number="newVariant.price" type="number" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-400" />
                                </div>
                                <div>
                                    <label class="text-[10px] font-semibold text-gray-500 block mb-1">Tồn kho</label>
                                    <input v-model.number="newVariant.stock_qty" type="number" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-400" />
                                </div>
                        ...
                        <div class="space-y-2">
                            <div v-for="v in selectedProduct.variants" :key="v.variant_id" class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                                <div class="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-xs font-bold text-gray-600 shrink-0">{{ v.size?.name }}</div>
                                <div class="w-4 h-4 rounded-full border border-gray-200 shrink-0" :style="{ backgroundColor: v.color?.hex_code || '#ccc' }"></div>
                                <span class="text-[10px] text-gray-400 font-mono shrink-0 hidden sm:block">{{ v.sku }}</span>
                                <div class="flex items-center gap-2 ml-auto">
                                    <div><p class="text-[9px] text-gray-400 mb-0.5">Giá</p><input v-model.number="v.price" type="number" class="w-20 bg-white border border-gray-200 rounded-lg px-2 py-1 text-xs outline-none focus:border-indigo-400" /></div>
                                    <div><p class="text-[9px] text-gray-400 mb-0.5">Kho</p><input v-model.number="v.stock_qty" type="number" min="0" class="w-14 bg-white border border-gray-200 rounded-lg px-2 py-1 text-xs outline-none focus:border-indigo-400" /></div>
                                    <div><p class="text-[9px] text-gray-400 mb-0.5">Ngưỡng</p><input v-model.number="v.low_stock_threshold" type="number" class="w-12 bg-white border border-gray-200 rounded-lg px-2 py-1 text-xs outline-none focus:border-indigo-400" /></div>
                                    <button @click="handleUpdateVariant(v)" class="p-2 hover:bg-indigo-50 rounded-lg transition-colors text-gray-400 hover:text-indigo-600 mt-3"><span class="material-symbols-outlined text-[18px]">save</span></button>
                                </div>
                            </div>
                        </div>
                    </section>
