  ...
  try {
    ...
    if (product.value.variants?.length > 0) {
      ...
      if (authStore.isAuthenticated && authStore.user) {
        const suggestedSizeName = getSuggestedSizeForProduct(
          product.value,
          authStore.user,
        );
...
const suggestedSizeName = computed(() => {
  if (!product.value || !authStore.isAuthenticated || !authStore.user)
    return null;
...
const fetchStockNotificationStatus = async () => {
  ...
  try {
    const response = await productService.getStockNotificationStatus(
      variant.variant_id,
    );
...
const handleBuyNow = async () => {
  ...
  try {
    ...
    if (!cartItem) {
      uiStore.error(
        "Không thể xác định sản phẩm cần thanh toán. Vui lòng thử lại.",
      );
      return;
    }
...
<template>
  ...
  <div
    ...
    class="min-h-screen flex items-center justify-center bg-background-light"
  >
    <div
      class="animate-spin h-8 w-8 border-2 border-black border-t-transparent rounded-full"
    ></div>
  </div>
  ...
  <div v-else-if="product" class="bg-background-light min-h-screen pb-24">
    <div class="max-w-[1400px] mx-auto px-6 md:px-12 pt-10">
      <!-- Breadcrumb -->
      <nav
        class="flex items-center gap-2 text-[10px] uppercase tracking-widest text-text-muted mb-8 font-display"
      >
        <router-link
          to="/"
        ...
        >
        <span class="text-border-light">/</span>
        <span class="text-fashion-black font-medium truncate max-w-[200px]">{{
          product.name
        }}</span>
      </nav>
      ...
      <div
        ...
        class="mt-20 pt-16 border-t border-border-light"
      >
        <div class="mb-10 text-center">
          <h3
            class="text-2xl md:text-3xl font-serif italic text-fashion-black mb-2"
          >
            Thường được mua kèm
          </h3>
          <p
            class="text-text-muted text-xs tracking-wider uppercase font-display"
          >
            Gợi ý dựa trên xu hướng mua sắm
          </p>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          >
            <!-- Image -->
            <div
              class="relative aspect-[3/4] overflow-hidden bg-fashion-gray rounded-xl mb-3 border border-border-light"
            >
              <img
                v-if="item.image_url"
              ...
              >
                <span
                  class="material-symbols-outlined text-4xl text-text-muted/30"
              ...
              <!-- Hover overlay -->
              <div
                class="absolute inset-0 bg-fashion-black/15 opacity-0 group-hover:opacity-100 transition-opacity duration-400"
              ></div>

              <!-- Quick view -->
              <div
                class="absolute inset-x-4 bottom-4 translate-y-3 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300"
              >
                <div
                  class="bg-white text-fashion-black text-[10px] font-bold text-center tracking-widest uppercase py-2.5 rounded-none font-display hover:bg-black hover:text-white transition-colors shadow-lg"
                >
                  Xem chi tiết
                </div>
              </div>
            ...
            <div class="space-y-1 px-0.5">
              <h4
                class="text-sm font-semibold text-fashion-black group-hover:text-black group-hover:underline transition-colors line-clamp-2 leading-snug font-display"
              >
                {{ item.name }}
              </h4>
              <p class="text-sm font-bold text-black font-display">
